...
- **Answer passive clarifications** — respond to the batched, high-leverage questions D2 cannot settle by investigation alone. The governing principle is **minimal Designer attention**: D2 does deep investigation *between* rounds, and though multiple rounds are permitted they are not encouraged. → **A-018**.
- **Review and confirm the D1 foundational documents** at a key Review Stop. → new action **A-057**.

**The common element — every D1 project needs its own constitution.** Whatever the project, the natural first thing to produce by combining the setup skeleton, the predecessor, and the intended change is the **D1 project's own foundational documents**: a D1-level analogue of the very Phases 1–5 that govern this D2 work — a **D1 Constitution** and the foundational set around it — scope, principles, invariants, initial direction, and the project's **rules and standards**. The rules and standards especially belong here: they are the active governing document the design work cannot proceed without, so most of them must be established now, not deferred. This is deeply self-similar: D2 is itself governed by a constitution, and its first act for a D1 project is to give that project a constitution of its own. That his input so often speaks *for the D0 users* — their complaints, the features they want, the behaviour that fails them — is why the D0-user considerations of Section 1 first enter the design here, in his own words rather than a form.
...
- **Produce the D1 foundational documents** — combine skeleton + predecessor + intent into the D1 Constitution and its foundational set (understanding, what to preserve / change / add, invariants, direction, residual uncertainties) **and the D1 rules and standards** — the active governing document the design cannot proceed without, so most rules are set here. → new capabilities **C-39** (foundational set), **C-40** (rules and standards).
...
**How D2 produces them — template plus merge.** D2 does not start from a blank page. It seeds the foundational set from a **default template modeled on D2's own foundational documents** (this very Phases 1–5 structure), giving the D1 project a proven starting shape the Designer then revises and extends — all such revisions and additions legitimate. For the rules and standards specifically, D2 **merges the predecessor's existing rules** (mined from the V1 system) **with the default rules the template provides**, reconciling them into the project's active rule set. The template is a starting point, not a straitjacket.
...
**The common element.** Every project's design work, whatever its content, needs the same things settled before momentum builds: a *plan of the work* and the *points where the Designer stays in control* — the process concerns of *how* D2 designs, distinct from the constitution's product concerns. The *rules the work will follow* are not originated here: they were already established as foundational documents in §4 (the work could not have proceeded otherwise), and this step only **refines** them where needed.
...
- **Any updates to the rules and standards** — the governing rules and standards were established in §4 (the design could not have proceeded otherwise); the framework references that active document and may **propose updates or additions** to it, under the same governance, but does not originate it.
